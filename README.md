...
The tracked `git/.gitconfig` ships only shared settings (aliases) — **no
name/email**. Your personal identity lives in `~/.gitconfig.local`, which is
gitignored so it's never published. Create it after installing:
...
`~/.gitconfig` includes this file automatically (git silently ignores it if
missing). Tip: use your GitHub `noreply` address
(`<id>+<user>@users.noreply.github.com`) to keep your real email out of commit
metadata.
